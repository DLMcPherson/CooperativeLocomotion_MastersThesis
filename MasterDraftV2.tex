...
% TODO: tie into Hamiltonian (assume momentum p is so low that T is negligible). Work (energy change) is force over distance (positional state), so force is related to energy differential of state. Do the maths.
% All change in overall energy must be caused by the non-conservative, exogenous pushing force. How much force/power is necessary? Try to minimize!
...
%TODO: splice in expressions for derivatives of l and c wrt theta
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.4\textwidth]{DynamicPromise1.png}
  \includegraphics[width=0.4\textwidth]{DynamicPromise2.png}
